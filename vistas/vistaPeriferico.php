...
<!--Nombre y Características-->
    <div class="container text-center"  style="margin-top:-100px;">
      <h1><?php nombrePeriferico(); ?></h1>
    </div>
    <br>

    <div class="container"  style="margin-top:;">
      <h3>Características del Periférico</h3>
      <hr>
      <?php caracteristicasPerif(); ?>
    </div>
  <!--Fin Nombre y Características-->

    <div class="container text-center">
    <form method="post" action="../back/eliminarPeriferico.php?id= <?php if(isset( $_GET['id'])){$id = $_GET['id']; echo $id; };?>">
      <button class="btn btn-danger" name="eliminarPeriferico">Eliminar Periferico</button>
    </form>
       <button class="btn btn-success" onclick="ver('registrarPeriferico', 'cancelarRegistro', 'guardarPeriferico')">Editar</button>
    </div>
  <br>

   <!--Formulario para editar periféricos-->
   <div class="container">
    <div class="container text-center">
      <form id="registrarPeriferico" method="post" action="../back/registrarPerifericos.php?id= <?php if(isset( $_GET['id'])){$id = $_GET['id']; echo $id; };?>">
        <?php editarPerifericos(); ?>
        <div class="text-center" id="guardarPeriferico">
          <button class="btn btn-success"  name="editarPeriferico">Guardar</button>
        </div>
      </form>
      <div class="container" id="cancelarRegistro">
        <a href="#equiposRegistrados"><button class="btn btn-danger" onclick="ocultar('cancelarRegistro', 'registrarPeriferico', 'guardarPeriferico')">Cancelar</button></a>
       </div>
    </div>
  </div>
